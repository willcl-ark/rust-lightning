	...
	fn is_resolving_htlc_output(&mut self, tx: &Transaction) -> Vec<(HTLCSource, Option<PaymentPreimage>, PaymentHash)> {
		...
		'outer_loop: for input in &tx.input {
			...
			macro_rules! scan_commitment {
				($htlcs: expr, $tx_info: expr) => {
					for (ref htlc_output, ref source_option) in $htlcs {
						if Some(input.previous_output.vout) == htlc_output.transaction_output_index {
							if let &Some(ref source) = source_option {
								log_trace!(self, "Input spending {}:{} resolves HTLC with payment hash {} from {}", input.previous_output.txid, input.previous_output.vout, log_bytes!(htlc_output.payment_hash.0), $tx_info);
								payment_data = Some(((*source).clone(), htlc_output.payment_hash));
							} else {
								log_info!(self, "Input spending {}:{} in {} resolves inbound HTLC with timeout from {}", input.previous_output.txid, input.previous_output.vout, tx.txid(), $tx_info);
								continue 'outer_loop;
							}
						}
					}
			...
			if let Some(ref current_local_signed_commitment_tx) = self.current_local_signed_commitment_tx {
				if input.previous_output.txid == current_local_signed_commitment_tx.txid {
					scan_commitment!(current_local_signed_commitment_tx.htlc_outputs.iter().map(|&(ref a, _, ref b)| (a, b)),
			...
			if let Some(ref prev_local_signed_commitment_tx) = self.prev_local_signed_commitment_tx {
				if input.previous_output.txid == prev_local_signed_commitment_tx.txid {
					scan_commitment!(prev_local_signed_commitment_tx.htlc_outputs.iter().map(|&(ref a, _, ref b)| (a, b)),
						"our latest local commitment tx");
				}
			}
			if let Some(ref htlc_outputs) = self.remote_claimable_outpoints.get(&input.previous_output.txid) {
				scan_commitment!(htlc_outputs.iter().map(|&(ref a, ref b)| (a, (b.as_ref().clone()).map(|boxed| &**boxed))), "remote commitment tx");
			}
